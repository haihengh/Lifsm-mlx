...
@dataclass
class ModelConfig:
    """Distilled from config.json — only fields FinchMoE engine needs."""
...
class FinchMoEConverter:
    """
    Orchestrates the full safetensors → .finchmoe conversion.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Convert Qwen3 MoE safetensors → .finchmoe",
    ...
    p.add_argument("--output",    required=True, type=Path,
                   help="Output .finchmoe file path")
