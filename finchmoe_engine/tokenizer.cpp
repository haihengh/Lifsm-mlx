...
std::unique_ptr<Tokenizer> make_tokenizer(const std::string& model_path) {
    // Replace .finchmoe extension with .tiktoken
    std::string vocab_path = model_path;
    const std::string finchmoe_ext = ".finchmoe";
    size_t ext_pos = vocab_path.rfind(finchmoe_ext);
    if (ext_pos != std::string::npos) {
        vocab_path.replace(ext_pos, finchmoe_ext.size(), ".tiktoken");
    } else {
        vocab_path += ".tiktoken";
    }
