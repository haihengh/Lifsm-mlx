// tools/chat_main.cpp
// ─────────────────────────────────────────────────────────────────────────────
// Interactive CLI chat demo for FinchMoE with tool-calling support.
//
// Usage
// ─────
//   ./finchmoe_chat <model.finchmoe> [options]
//
// Options
...
static void print_usage(const char* argv0) {
    fprintf(stderr,
        "Usage: %s <model.finchmoe> [options]\n"
        "\n"
        "Options:\n"
...
int main(int argc, char* argv[]) {
    ...
    if (!tokenizer) {
        fprintf(stderr, "[chat] Failed to load tokenizer.\n"
                        "       Expected: %s.tiktoken\n"
                        "       (run finchmoe_convert.py to generate it)\n",
                model_path.c_str());
        return 1;
    }
