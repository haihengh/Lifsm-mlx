# AeroMoE Engine — Complete Reference
...
./build/aeromoe_generate  qwen3.aeromoe  "Explain MoE routing in one paragraph"
...
├── aeromoe_types.h              DType, ModelConfig, TensorView, Status
├── aeromoe_format.h             .aeromoe file parser: header + dense/expert indexes
...
```
aeromoe_convert.py               safetensors → .aeromoe weight converter
```
...
|--------|---------|-------------|
| `aeromoe_engine` | *(static lib)* | All engine sources; linked by the executables |
| `aeromoe_test` | `cmake --build build -t aeromoe_test` | Smoke test: load + generate 20 tokens |
| `aeromoe_generate` | `cmake --build build -t aeromoe_generate` | One-shot generate CLI |
| `aeromoe_chat` | `cmake --build build -t aeromoe_chat` | Interactive REPL with tool-calling |
