// aeromoe_format.h
// ─────────────────────────────────────────────────────────────────────────────
// .aeromoe file parsing: header, dense index, expert index.
...
static constexpr uint8_t  MAGIC[8]         = {'A','E','R','O','M','O','E','\0'};
...
// ── AeroMoEFile: open, validate, expose indexes ───────────────────────────────
...
public:
    ...
    AeroMoEFile(const AeroMoEFile&) = delete;
    AeroMoEFile& operator=(const AeroMoEFile&) = delete;
    AeroMoEFile(AeroMoEFile&& o) noexcept : fd_(o.fd_) { o.fd_ = -1; }
    ...
    Status open(const std::string& path) {
        ...
        if (fd_ < 0) {
            fprintf(stderr, "[aeromoe] Failed to open %s: %s\n",
        ...
        fprintf(stderr, "[aeromoe] Opened %s  (%zu GB)\n"
                "          Dense tensors : %zu\n"
                "          Expert slots  : %zu\n",
...
private:
    ...
    Status read_header() {
        ...
        if (memcmp(hdr.magic, MAGIC, 8) != 0) {
            fprintf(stderr, "[aeromoe] Bad magic bytes\n");
            return Status::FormatError;
        }
        if (hdr.version != FORMAT_VERSION) {
            fprintf(stderr, "[aeromoe] Version mismatch: got 0x%08X want 0x%08X\n",
