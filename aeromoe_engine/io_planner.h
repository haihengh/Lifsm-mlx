...
//
// The IOPlanner does NOT own any buffers. It only reads bytes from the .aeromoe
...
class IOPlanner {
public:
    explicit IOPlanner(const format::AeroMoEFile& file,
                       int num_threads = 4);
    ~IOPlanner();
...
private:
    ...
    const format::AeroMoEFile& file_;
